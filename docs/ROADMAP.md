...
built all six. **Thirty-five verbs across seven nouns**, and the helper that answered
...
| M3.10 | **The suite covers what the tool grew** (`e2e/`) | **done 2026-09-03** — eighteen checks, about ten minutes. Client nodes with their two mounts and the broker path, two clients sharing one rate, the CTP fragment both ways, `scenario run` against its own cluster, and a tailnet check that skips unless an auth key is set. It found **four defects on its first run, two of them in the product**: the key ranges had a lower bound and no upper one, so a resuming driver read the global `MAX(i)` and walked into another client's keys — 200 statements out of 204 on the second run against a table with rows in it; and `WriteCTPConf` wrote client nodes into an `ha_repl.conf` as slaves. Then a second run found a third: **`node start` and `ha promote` never restarted the broker**, so a cluster created `--with-broker` came back from a failover serving and unreachable through its own door, which nothing had noticed until a client tried to use it |
...
| M3.11 | **What the surface asks of a first-time user** | **done 2026-09-07** — three things a new reader hit and the roadmap had no line for. `--help` was scanned for rather than resolved, so every `--help` printed the whole surface and the **forty-five flags each command declares were reachable only from the README** — and `node exec master -- csql --help` answered csql's question with our usage instead of running the command. Help is now resolved against the registry, the global page carries the selector grammar (§2) and the exit codes (§6), and a test walks the registry so a flag cannot go back to being invisible. A **scenario silently dropped unknown keys**: `contain` for `contains` produced a step that ran, asserted nothing and printed `ok`, which is the one result a tool like this cannot have — the rule the tool already applied to a CTP conf it did not write now applies to its own format, and every step's argv, duration, `master_is`, `measure` name and `${binding}` is checked before `cluster create` rather than nine steps and two minutes in. And **docker was never asked whether it could be used**: the first failure was whichever docker command the assembly reached first, exit 1, with a daemon that is down indistinguishable from a socket this user cannot open. One `docker version` now separates the three, exit 3. The smallest of the four is the most likely to be hit: `--build ~/cubrid` is what muscle memory types, and the tool can see that `install.out` is sitting in there |
